...
from Lab5_student_GR1 import planner, steering_angle
...
if __name__ == "__main__":
    ...
    tracker = aruco_track()
    axes_origin_ID = 16  #int(input('Enter origin ID: 16'))
    car_ID = 3   #int(input('Enter car ID: 3'))
    goal_ID = 11   #int(input('Enter goal ID: 4'))
    ...
    obs = [np.hstack(((np.linalg.inv(homo[axes_origin_ID])@homo[i])[:2, 3], 0.06)).tolist() for i in obs]
